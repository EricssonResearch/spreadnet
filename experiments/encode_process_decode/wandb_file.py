    ...
    try:
        ...
        for epoch in range(epochs):
            ...
            validation_acc = execute(
                "validation", validation_loader, model, hybrid_loss
            )
            ...
            wandb.log(
                {
                    "Train Loss": {
                        "Node": losses_curve[-1]["nodes"],
                        "Edge": losses_curve[-1]["edges"],
                    },
                    "Validation Loss": validation_losses_curve[-1]["nodes"],
                    "epoch": epoch,
                },
                commit=True,
            )
            ...
            wandb.log(
                {
                    "Train Acc": {
                        "Node": accuracies_curve[-1]["nodes"],
                        "Edge": accuracies_curve[-1]["edges"],
                    },
                    "Validation Acc": validation_accuracies_curve[-1]["edges"],
                    "epoch": epoch,
                },
                commit=True,
            )
